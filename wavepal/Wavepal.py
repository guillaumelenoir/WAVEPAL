...
class Wavepal:
	...
	def plot_periodogram(self,xaxis="frequency",loglog="no",fontsize_title=14,fontsize_axes=12,fontsize_ticks=12,fontsize_legend='xx-small',linewidth_per=1.0,linewidth_cl=1.0):
		...
		for k in range(self.percentile.size):
			if 'n' in self.signif_level_type:
				if loglog=="no":
					plt.plot(self.freq**mypow,self.periodogram_cl_mcmc[:,k],label="Conf. level from full MCMC - "+str(self.percentile[k])+"%",linewidth=linewidth_cl)
				elif loglog=="yes":
					plt.loglog(self.freq**mypow,self.periodogram_cl_mcmc[:,k],label="Conf. level from full MCMC - "+str(self.percentile[k])+"%",linewidth=linewidth_cl)
			if 'a' in self.signif_level_type:
				if self.p==0:
					if loglog=="no":
						plt.plot(self.freq**mypow,self.periodogram_cl_anal[:,k],label="Conf. level from the max of the posterior pdf - "+str(self.percentile[k])+"%",linewidth=linewidth_cl)
					elif loglog=="yes":
						plt.loglog(self.freq**mypow,self.periodogram_cl_anal[:,k],label="Conf. level from the max of the posterior pdf - "+str(self.percentile[k])+"%",linewidth=linewidth_cl)
				else:
					if loglog=="no":
						plt.plot(self.freq**mypow,self.periodogram_cl_anal[:,k],label="Conf. level from the median coeffs - "+str(self.percentile[k])+"%",linewidth=linewidth_cl)
					elif loglog=="yes":
						plt.loglog(self.freq**mypow,self.periodogram_cl_anal[:,k],label="Conf. level from the median coeffs - "+str(self.percentile[k])+"%",linewidth=linewidth_cl)
	...
	def plot_check_convergence_percentiles(self,display="frequency",fontsize_suptitle=18,fontsize_title=14,fontsize_axes=12,fontsize_ticks=12,fontsize_legend='small',linewidth=1.0):
		...
		plt.suptitle("Convergence of the percentiles", fontsize=fontsize_suptitle)
		for l in range(6):
			if l==0:
				aa=axarr[0,0]
			elif l==1:
	...
	def plot_amplitude(self,xaxis="frequency",loglog="no",fontsize_title=14,fontsize_axes=12,fontsize_ticks=12,plot_band_filtering="no",linewidth=1.0):
		...
		if xaxis=="period":
			...
			mypow=1
		if loglog=="no":
			plt.plot(self.freq**mypow,self.amplitude,label="Amplitude",linewidth=linewidth)
		elif loglog=="yes":
			plt.loglog(self.freq**mypow,self.amplitude,label="Amplitude",linewidth=linewidth)
	...
	def plot_global_scalogram(self,xaxis="period",loglog="no",fontsize_title=14,fontsize_axes=12,fontsize_ticks=12,fontsize_legend='xx-small',linewidth_gscal=1.0,linewidth_gcl=1.0):
		...
		for k in range(self.percentile_cwt.size):
			if 'n' in self.signif_level_type:
				if loglog=="no":
					plt.plot(self.period_cwt**mypow,self.global_scalogram_cl_mcmc[:,k],label="Conf. level from full MCMC - "+str(self.percentile_cwt[k])+"%",linewidth=linewidth_gcl)
				elif loglog=="yes":
					plt.loglog(self.period_cwt**mypow,self.global_scalogram_cl_mcmc[:,k],label="Conf. level from full MCMC - "+str(self.percentile_cwt[k])+"%",linewidth=linewidth_gcl)
			if 'a' in self.signif_level_type:
				if self.p==0:
					if loglog=="no":
						 plt.plot(self.period_cwt**mypow,self.global_scalogram_cl_anal[:,k],label="Conf. level from the max of the posterior pdf - "+str(self.percentile_cwt[k])+"%",linewidth=linewidth_gcl)
					elif loglog=="yes":
						 plt.loglog(self.period_cwt**mypow,self.global_scalogram_cl_anal[:,k],label="Conf. level from the max of the posterior pdf - "+str(self.percentile_cwt[k])+"%",linewidth=linewidth_gcl)
				else:
					if loglog=="no":
						 plt.plot(self.period_cwt**mypow,self.global_scalogram_cl_anal[:,k],label="Conf. level from the median coeffs - "+str(self.percentile_cwt[k])+"%",linewidth=linewidth_gcl)
					elif loglog=="yes":
						 plt.loglog(self.period_cwt**mypow,self.global_scalogram_cl_anal[:,k],label="Conf. level from the median coeffs - "+str(self.percentile_cwt[k])+"%",linewidth=linewidth_gcl)
		plt.legend(fancybox=True,fontsize=fontsize_legend,bbox_to_anchor=(1.1, 1.05))
		if xaxis=="period":
			plt.xlabel("Period"+self.t_label,fontsize=fontsize_axes)
		elif xaxis=="frequency":
